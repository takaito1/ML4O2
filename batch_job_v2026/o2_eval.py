...
if alg=='RF':
    ...
    xticks=['5x2','10x2','20x2','40x2','20-10','20-15-10-5']
...
for n in range(6):
    ...
    if alg=='RF':
        ...
        print(f'The #{n+1} best performing case is with hidden layers={xx[ind][0]} and regularization={yy[ind][0]} with the mean K-fold RMSE={np.round(tmp1[ind][0],4)}')
...
parm2 = yy[ranks==0][0]
print(parm1,parm2)
np.savez(dirout+f'optparm_v{ver}.npz',parm1=parm1,parm2=parm2)
...
def rev_trans_validation(basin,nver):
    ...
    # if not in the Southern Ocean, return the whole basin info.
        ...
        RMSE = np.sqrt(np.mean((o2est-o2test)**2))
    #
    return R2,RMSE,lati,long180,Z,o2est,o2test,sect
...
#
R2,RMSE,lat,lon,depth,o2est,o2test,sect = rev_trans_validation(basin,0)
fig=plt.figure(figsize=[7,6])
if basin==4:
...
#plt.close()
tmp=np.load(f'{dirout}train_test_v{ver}.npz')
yr_drop = tmp['yr_drop']
print(f'dropped years = {yr_drop}')
...
ax.set_title(f'validation, version {ver}')
